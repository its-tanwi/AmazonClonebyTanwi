...
const Products=({productData} : any)=>{
    return (<div className="w-full px-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
{productData.map(({id,
            title,
            price,
            description,
                ...
                <div key={id} className="w-full bg-white text-black p-4 border border-gray-300
                    ...
                    <div className="w-full h-[260px] relative">
                        ...
                        transition-transform duration-300">
                            <span className="w-full h-full border-b-[1px] border-b-gray-400 flex items-center justify-center 
...
"><FaHeart/></span>
                    ...
                    <p className="ext-base font-medium">{title}</p>
                    <p className="flex items-center">
                        <span className="text-amazon_blue font-bold">
                        <FormattedPrice amount={price*10}/></span></p>
                    <p className="text-xs text-gray-500 text-justify">
                        {description.substring(0,200)}</p>
                        <button className="h-10 font-medium bg-amazon_blue text-white round-md hover:bg-amazon_yellow 
